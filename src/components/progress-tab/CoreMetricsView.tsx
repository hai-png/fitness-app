/**
 * CoreMetricsView — extracted from ProgressTab.tsx (A-04 decomposition).
 *
 * Sub-tab 1: Overall volume, rolling periodic velocity, training focus
 * splits, 1RM estimator widget, and configurable secondary-muscle load
 * multiplier. Receives the parent's memoised analytics bundle + the
 * multiplier state it owns.
 */
...
export default function CoreMetricsView({
  analytics,
  multiplierSecondary,
  setMultiplierSecondary,
}: CoreMetricsViewProps) {
  const { filteredLogs, coreMetrics, rollingTrends } = analytics;

/**
 * @file MuscleMapTab.tsx — Sub-tab 2 of the Progress / Analytics view.
 *
 * Renders the "Muscle Map" sub-tab inside <ProgressTab />:
 *   - Adaptable Training Age selector (Beginner / Intermediate / Advanced)
 *     that shifts MEV / MRV thresholds for every muscle zone.
 *   - Interactive Anatomical Load Map: a left-side button list of every muscle
 *     zone with hover/selected states, paired with a right-side details panel
 *     that shows weekly sets, the Stimulus Zone badge (MEV/MAV/MRV/etc.), the
 *     hypertrophy score (0-100), and the muscle's share of total volume.
 *   - Volume Distribution Balance panel: flags asymmetry if the top-3 muscles
 *     account for >70% of weekly volume, plus a per-muscle bar list.
 *   - Lifetime Mass Output panel: tier name, tons lifted lifetime, progress
 *     toward the next tier, and weeks-to-graduate estimate.
 *
 * Pure presentational component — all values flow in via props.
 */
import { Sliders, User, Award } from "lucide-react";
import {
  MuscleVolumeZone,
} from "../../data/analyticsEngine";

/** Result of the muscle-balance useMemo in ProgressTab. */
...
/** Result of the lifetime-tier useMemo in ProgressTab. */
...
export interface MuscleMapTabProps {
  /** Per-muscle weekly sets, zone, score, and balance share. */
  muscleZonesAndScores: MuscleVolumeZone[];
  /** Currently selected muscle name (or null). */
  selectedMuscle: string | null;
  /** Setter for `selectedMuscle`. */
  setSelectedMuscle: (v: string | null) => void;
  /** Currently hovered muscle name (or null). */
  hoveredMuscle: string | null;
  /** Setter for `hoveredMuscle`. */
  setHoveredMuscle: (v: string | null) => void;
  /** Active training age — shifts MEV/MRV thresholds. */
  trainingAge: "Beginner" | "Intermediate" | "Advanced";
  /** Setter for `trainingAge`. */
  setTrainingAge: (v: "Beginner" | "Intermediate" | "Advanced") => void;
  /** Pre-computed balance summary (top-3 share + imbalanced flag + sorted list). */
  muscleBalanceAnalysis: MuscleBalanceAnalysis;
  /** Pre-computed lifetime tier progress summary. */
  lifetimeTierInfo: LifetimeTierInfo;
  /** Total working mass lifted lifetime, in tons. */
  lifetimeVolumeTons: number;
}

/**
 * Renders the "Muscle Map" sub-tab.
 *
 * Behaviour is identical to the original `renderMuscleMapTab()` closure
 * inside ProgressTab.tsx — only the wrapping has changed (closure → component).
 */
export function MuscleMapTab({

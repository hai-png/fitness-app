...
export default function ProgressTab({
  ...
  const [activeSubTab, setActiveSubTab] = useState<"metrics" | "muscles" | "exercises" | "visuals">("metrics");
  ...
  // --- INITIALIZATION ---
  useEffect(() => {
    // Load historical exercise set logs from localStorage or generate pre-seeded on first run
    const stored = localStorage.getItem("aether_exercise_set_logs");
    if (stored) {
      try {
        setExerciseLogs(JSON.parse(stored));
      } catch (e) {
        const generated = generateWorkoutHistory();
        setExerciseLogs(generated);
        localStorage.setItem("aether_exercise_set_logs", JSON.stringify(generated));
      }
    } else {
      const generated = generateWorkoutHistory();
      setExerciseLogs(generated);
  ...
  const saveLogsToStorage = (updated: ExerciseLog[]) => {
    ...
    localStorage.setItem("aether_exercise_set_logs", JSON.stringify(updated));
  };

  // Reset exercise database back to original pre-seeded data
  const handleResetHistory = () => {
    if (confirm("Are you sure you want to reset your training metrics? This will restore the pre-seeded progressive logs.")) {
      const generated = generateWorkoutHistory();
      saveLogsToStorage(generated);
      setDateFilterStart("");
      setDateFilterEnd("");
    }
  };
  ...
  const lifetimeTierInfo = useMemo(() => {
    ...
    const recent4WeeksVolume = filteredLogs
      .filter((l) => {
        ...
        const fourWeeksAgo = new Date("2026-06-29");
        fourWeeksAgo.setDate(fourWeeksAgo.getDate() - 28);
        return d >= fourWeeksAgo;
      })
      .reduce((sum, ex) => sum + ex.sets.reduce((sSum, s) => sSum + (s.isWarmUp ? 0 : s.weight * s.reps), 0), 0);
    
    const avgWeeklyTons = (recent4WeeksVolume / 4) / 1000; // tons/week
    const weeksToNext = avgWeeklyTons > 0 && tonsToNext > 0 ? Math.ceil(tonsToNext / avgWeeklyTons) : 100;
  ...
  const handleLogCustomSetSubmit = (e: React.FormEvent) => {
    ...
    const existingIndex = updated.findIndex((l) => l.exerciseName === logExName && l.date === todayStr);
    ...
    alert(`💪 Successfully logged set: ${weight}kg x ${reps} reps for ${logExName}!`);
  };

  // Muscle Category to Muscle Groups mapper
  const handleMuscleCategoryChange = (cat: string) => {
    setLogExMuscle(cat);
    const firstExInCat = EXERCISE_DATABASE.find(e => e.targetMuscle.toLowerCase() === cat.toLowerCase());
    if (firstExInCat) {
  ...
  const renderCoreMetricsTab = () => {
    ...
    const hypertrophyPct = totalWorking > 0 ? Math.round((hypertrophySets / totalWorking) * 100) : 0;
      ...
      <div className="space-y-5 animate-fadeIn">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm relative overflow-hidden">
            ...
            <div className="text-xl font-serif font-black italic tracking-tight text-[#1A1A1A]">
              {coreMetrics.totalVolume.toLocaleString()} <span className="text-xs not-italic font-sans font-medium text-[#1A1A1A]/50">kg</span>
            </div>
            <p className="text-[10px] text-[#1A1A1A]/60 font-serif italic mt-1 leading-normal">
              Based on {coreMetrics.totalWorkingSets} working sets (excl. warm-ups) with secondary muscles @ {multiplierSecondary}x.
            </p>
          </div>
          ...
          <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm relative overflow-hidden">
            ...
            <div className="text-xl font-serif font-black italic tracking-tight text-[#1A1A1A]">
              {coreMetrics.volumePerMinute} <span className="text-xs not-italic font-sans font-medium text-[#1A1A1A]/50">kg/min</span>
            </div>
            <p className="text-[10px] text-[#1A1A1A]/60 font-serif italic mt-1 leading-normal">
              Accumulating mechanical tension inside a total session length of {coreMetrics.totalDuration} minutes.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm">
          ...
          <p className="text-[10px] text-[#1A1A1A]/60 mb-4 font-serif italic leading-relaxed">
            Compares absolute muscular load lifted across recent cycles against prior identical windows to verify systematic velocity.
          </p>

          <div className="grid grid-cols-3 gap-2 text-center">
            {/* 7 Days */}
            <div className="bg-[#F9F8F6] border border-[#1A1A1A]/5 p-2.5">
              <span className="text-[8px] uppercase tracking-wider font-bold text-[#1A1A1A]/40 block">Last 7 Days</span>
              <span className="text-xs font-mono font-bold block mt-1 text-[#1A1A1A]">
                {(rollingTrends.vol7 / 1000).toFixed(1)}t
              </span>
              <span className={`text-[9px] font-bold inline-flex items-center gap-0.5 mt-1 font-mono ${
                rollingTrends.diff7 >= 0 ? "text-emerald-700" : "text-rose-700"
              }`}>
                {rollingTrends.diff7 >= 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                {rollingTrends.diff7 >= 0 ? "+" : ""}{rollingTrends.diff7.toFixed(1)}%
              </span>
            </div>

            {/* 30 Days */}
            <div className="bg-[#F9F8F6] border border-[#1A1A1A]/5 p-2.5">
              <span className="text-[8px] uppercase tracking-wider font-bold text-[#1A1A1A]/40 block">Last 30 Days</span>
              <span className="text-xs font-mono font-bold block mt-1 text-[#1A1A1A]">
              ...
              <span className={`text-[9px] font-bold inline-flex items-center gap-0.5 mt-1 font-mono ${
                ...
                {rollingTrends.diff30 >= 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                {rollingTrends.diff30 >= 0 ? "+" : ""}{rollingTrends.diff30.toFixed(1)}%
              </span>
            </div>

            {/* 365 Days */}
            <div className="bg-[#F9F8F6] border border-[#1A1A1A]/5 p-2.5">
              <span className="text-[8px] uppercase tracking-wider font-bold text-[#1A1A1A]/40 block">Last Year</span>
              <span className="text-xs font-mono font-bold block mt-1 text-[#1A1A1A]">
              ...
              <span className={`text-[9px] font-bold inline-flex items-center gap-0.5 mt-1 font-mono ${
                ...
                {rollingTrends.diff365 >= 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                {rollingTrends.diff365 >= 0 ? "+" : ""}{rollingTrends.diff365.toFixed(1)}%
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm">
          ...
          <p className="text-[10px] text-[#1A1A1A]/60 mb-4 font-serif italic leading-relaxed">
            Classifies working sets by target fiber recruitment: Strength (1-5 reps), Hypertrophy (6-12 reps), and Endurance (13+ reps).
          </p>
          ...
          <div className="space-y-3.5">
            ...
            <div>
              ...
              <div className="h-2 bg-[#1A1A1A]/5 overflow-hidden">
                <div className="h-full bg-[#E63946] transition-all" style={{ width: `${strengthPct}%` }} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="h-2 bg-[#1A1A1A]/5 overflow-hidden">
                <div className="h-full bg-indigo-900 transition-all" style={{ width: `${hypertrophyPct}%` }} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="h-2 bg-[#1A1A1A]/5 overflow-hidden">
                <div className="h-full bg-emerald-700 transition-all" style={{ width: `${endurancePct}%` }} />
              </div>
            </div>
          </div>
        </div>

        {/* 1RM Estimation Calculator Widget */}
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-2 items-end">
            <div>
              <label className="block text-[8px] font-bold uppercase text-[#1A1A1A]/50 mb-1">Weight (kg)</label>
              <input 
                type="number" 
                defaultValue="80"
                id="1rm-weight"
                onChange={() => {}} // calculated on render via refs or value hooks
                className="w-full bg-[#F9F8F6] border border-[#1A1A1A]/10 p-1.5 text-xs focus:outline-none focus:border-[#E63946]"
              />
            </div>
            <div>
              <label className="block text-[8px] font-bold uppercase text-[#1A1A1A]/50 mb-1">Reps</label>
              <input 
                type="number" 
                defaultValue="5"
                id="1rm-reps"
            ...
            <button
              ...
              onClick={() => {
                const w = parseFloat((document.getElementById("1rm-weight") as HTMLInputElement)?.value || "80");
                const r = parseInt((document.getElementById("1rm-reps") as HTMLInputElement)?.value || "5");
                const result = calculateEpley1RM(w, r);
                alert(`🎯 Epley Formula Estimation: Your estimated 1-Rep Max (1RM) is ${Math.round(result)} kg (at ${w}kg for ${r} reps).`);
              }}
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-3.5 rounded-none shadow-sm flex items-center justify-between gap-4 text-xs">
          <div>
            <span className="font-bold text-[#1A1A1A] uppercase tracking-tight block text-[10.5px]">Configure Load Parameters</span>
            <span className="text-[9px] text-[#1A1A1A]/50 italic block mt-0.5">Adjust synergy coefficients for secondary arms/shoulders.</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-mono text-[#E63946] font-bold">{multiplierSecondary}x</span>
            <input 
  ...
  const renderMuscleMapTab = () => {
    const selectedMuscleData = muscleZonesAndScores.find((z) => z.muscle.toLowerCase() === selectedMuscle?.toLowerCase()) || 
      ...
      <div className="space-y-5 animate-fadeIn">
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <div>
              <span className="text-[8px] font-bold uppercase tracking-widest text-[#E63946] block">Zone Personalization</span>
              <h3 className="font-serif font-bold text-sm text-[#1A1A1A] mt-0.5">Adaptable Training Age</h3>
            </div>
            <select
          ...
          <p className="text-[10px] text-[#1A1A1A]/60 font-serif italic leading-relaxed">
            Training experience shifts your muscular adaptations limits. Minimum Effective Volume (MEV) and Max Recoverable Volume (MRV) zones are dynamically customized based on this setting.
          </p>
        </div>
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm relative">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1 max-h-[220px] overflow-y-auto pr-1">
              <span className="text-[8px] font-mono font-bold text-[#1A1A1A]/40 block uppercase tracking-wider mb-1.5">Interactive Muscle Zones</span>
            ...
            <div className="bg-[#F9F8F6] border border-[#1A1A1A]/5 p-3.5 flex flex-col justify-between h-[220px] relative overflow-hidden">
              ...
              <div>
                <span className="text-[7.5px] font-bold font-mono uppercase tracking-[0.2em] text-[#E63946] block">ACTIVE SECTOR DETAILS</span>
                ...
                <div className="mt-3 space-y-2">
                  <div>
                    <span className="text-[8px] font-mono font-bold text-[#1A1A1A]/40 block uppercase">Weekly Sets Rate</span>
                    <span className="text-xs font-mono font-bold text-[#1A1A1A]">{selectedMuscleData.weeklySets} working sets/week</span>
                  </div>

                  <div>
                    <span className="text-[8px] font-mono font-bold text-[#1A1A1A]/40 block uppercase mb-0.5">Stimulus Zone Badge</span>
                    <span className={`text-[9.5px] font-mono font-bold px-1.5 py-0.5 border uppercase tracking-wider block text-center rounded ${selectedMuscleData.colorClass}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1 border-t border-[#1A1A1A]/5">
                    <div>
                      <span className="text-[8px] font-mono font-bold text-[#1A1A1A]/40 block uppercase">Hypertrophy Score</span>
                      <span className="text-xs font-mono font-bold text-[#E63946]">{selectedMuscleData.score}/100</span>
                    </div>
                    <div>
                      <span className="text-[8px] font-mono font-bold text-[#1A1A1A]/40 block uppercase">Load Share</span>
                      <span className="text-xs font-mono font-bold text-[#1A1A1A]">{selectedMuscleData.balancePct}%</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <p className="text-[9px] text-[#1A1A1A]/55 font-serif italic leading-tight border-t border-[#1A1A1A]/5 pt-2 mt-2">
                {selectedMuscleData.zone.includes("MEV") ? "Optimal minimal effective stimulus for growth." : 
                 selectedMuscleData.zone.includes("MAV") ? "Perfect adaptive hypertrophy stimulus. Keep pushing!" :
                 selectedMuscleData.zone.includes("MRV") ? "Warning: High recoverability exhaustion risk. Deload suggested." :
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm">
          ...
          <p className="text-[10px] text-[#1A1A1A]/60 mb-3 font-serif italic leading-relaxed">
            Identifies muscular asymmetry issues. Standard balanced ratio flags if the top 3 muscle groups account for &gt;70% of total weekly volume.
          </p>

          <div className="border border-[#1A1A1A]/5 bg-[#F9F8F6] p-3 mb-4 rounded flex items-center justify-between">
            <div>
              <span className="text-[8px] font-mono font-bold uppercase text-[#1A1A1A]/40 block">Top 3 Muscle Concentration</span>
              <span className="text-sm font-mono font-black text-[#1A1A1A]">{muscleBalanceAnalysis.top3Share}% of total</span>
            </div>
          ...
          <div className="space-y-1 max-h-32 overflow-y-auto pr-1">
            {muscleBalanceAnalysis.sortedMuscles.slice(0, 5).map((m, idx) => (
              <div key={m.muscle} className="flex items-center justify-between text-[11px] font-mono py-1 border-b border-[#1A1A1A]/5">
                <span className="font-bold text-[#1A1A1A]/70">{idx + 1}. {m.muscle}</span>
                <div className="flex items-center gap-2">
        ...
        <div className="bg-[#1A1A1A] text-white p-4 rounded-none shadow-md relative overflow-hidden">
          ...
          <div className="flex justify-between items-start mb-2">
            <div>
              <span className="text-[7.5px] font-mono font-bold uppercase tracking-[0.25em] text-[#E63946]">LIFETIME MASSIVE OUTPUT</span>
          ...
          <div className="text-2xl font-mono font-black text-white">
            {lifetimeVolumeTons.toFixed(1)} <span className="text-xs font-sans font-bold text-white/50">Tons Lifted</span>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-[#E63946]" style={{ width: `${lifetimeTierInfo.progressPercent}%` }} />
            </div>
          </div>

          <p className="text-[10px] text-white/60 font-serif italic mt-3 leading-normal border-t border-white/5 pt-2">
            Based on current weekly volume, you are estimated to graduate to the <strong>{lifetimeTierInfo.next}</strong> tier in approximately <strong>{lifetimeTierInfo.weeksToNext}</strong> weeks.
          </p>
        </div>
      </div>
    );
  ...
  const renderExercisesTab = () => {
    ...
    const selectedExAnalysis = exerciseProgressions.find((e) => e.name === selectedAnalysisEx) || exerciseProgressions[0];
    ...
    const renderTrendChart = () => {
      if (selectedExHistory.length < 2) {
        return (
          <div className="flex flex-col items-center justify-center h-28 text-[#1A1A1A]/40 text-xs font-serif italic border border-[#1A1A1A]/5 bg-[#F9F8F6] p-3 text-center">
            ...
            <span>Generate multiple sessions of {selectedAnalysisEx} to render trend trajectories.</span>
          </div>
        );
      }
      ...
      const rawPoints = selectedExHistory.map((sess, idx) => {
        const maxWeight = Math.max(...sess.sets.filter(s => !s.isWarmUp).map(s => s.weight));
      ...
      const minW = Math.min(...weightsList) - 2.5;
      const maxW = Math.max(...weightsList) + 2.5;
      ...
      return (
        <div className="relative border border-[#1A1A1A]/10 p-3 bg-[#F9F8F6] rounded-none">
          <div className="flex justify-between items-center text-[8px] font-mono text-[#1A1A1A]/40 uppercase mb-2">
            ...
            <span className="text-[#E63946]">{isSmoothedTrend ? "Stable (Smoothed Moving Avg)" : "Reactive (Raw Peak)"}</span>
          </div>

          <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-24 overflow-visible">
            {/* Guidelines */}
            <line x1="0" y1={10} x2={width} y2={10} stroke="#1A1A1A" strokeOpacity="0.06" strokeDasharray="2 2" />
            <line x1="0" y1={height - 10} x2={width} y2={height - 10} stroke="#1A1A1A" strokeOpacity="0.06" strokeDasharray="2 2" />

            {/* Line Path */}
            <path d={path} fill="none" stroke="#E63946" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />

            {/* Nodes */}
            {coords.map((c, i) => (
              <g key={i}>
                <circle cx={c.x} cy={c.y} r="2.5" fill="#FFFFFF" stroke="#E63946" strokeWidth="1.5" />
                {(i === 0 || i === coords.length - 1) && (
                  <text x={c.x} y={c.y - 7} textAnchor="middle" fill="#1A1A1A" fontSize="7" fontWeight="bold" className="font-mono">
                    {c.weight}kg
                  </text>
                )}
              </g>
    ...
    return (
      <div className="space-y-5 animate-fadeIn">
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm">
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-serif font-bold text-sm text-[#1A1A1A]">Trend Trajectory analysis</h3>
            <select
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className={`text-[8.5px] font-mono font-bold px-1.5 py-0.5 border uppercase rounded ${
                    selectedExAnalysis.statusLabel === "Accelerating" ? "bg-emerald-50 text-emerald-800 border-emerald-100" :
                    selectedExAnalysis.statusLabel === "Slipping" ? "bg-rose-50 text-rose-800 border-rose-100" :
                  ...
                  <span className="text-[9px] font-mono text-[#1A1A1A]/45">
                    Confidence: {selectedExAnalysis.confidence} ({selectedExAnalysis.sessionCount} sessions)
                  </span>
                </div>
              ...
              {selectedPR && (
                <div className="grid grid-cols-2 gap-2 text-xs font-mono border-t border-[#1A1A1A]/5 pt-3">
                  <div>
                    <span className="text-[8px] text-[#1A1A1A]/40 uppercase block">Gold PR (All-Time)</span>
                    <span className="font-bold text-[#1A1A1A]">{selectedPR.goldValue}kg</span>
                    <span className="text-[8.5px] text-[#1A1A1A]/45 block italic mt-0.5">{selectedPR.dateGold}</span>
                  </div>
                  <div>
                    <span className="text-[8px] text-[#1A1A1A]/40 uppercase block">Silver PR (Last 60 Days)</span>
                    <span className="font-bold text-[#E63946]">{selectedPR.silverValue}kg</span>
                    <span className="text-[8.5px] text-[#1A1A1A]/45 block italic mt-0.5">{selectedPR.dateSilver}</span>
                  </div>
                  ...
                  {selectedPR.prematureFlagged && (
                    <div className="col-span-2 bg-amber-50 text-amber-800 border border-amber-100 p-2.5 rounded-none text-[10.5px] mt-1.5 leading-normal">
                      <span className="font-bold uppercase tracking-wide block text-[8px] text-amber-700">⚠️ Premature Peak Detected</span>
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm">
          ...
          <p className="text-[10px] text-[#1A1A1A]/60 mb-3 font-serif italic leading-relaxed">
            Checks if top weights have stalled or deteriorated across consecutive sessions, and provides specific athletic adjustments.
          </p>
          ...
          {plateauExercises.length === 0 ? (
            <div className="text-center py-4 bg-[#F9F8F6] border border-[#1A1A1A]/5 text-xs text-[#1A1A1A]/40 font-serif italic">
              No flat stall lines flagged! Consistent progressive overload detected across active movements.
            </div>
          ) : (
              ...
              {plateauExercises.map((p) => (
                <div key={p.name} className="bg-amber-50/40 border border-amber-100 p-3 text-xs">
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-bold text-[#1A1A1A] uppercase tracking-tight text-[11px]">{p.name}</span>
                    <span className="text-[8px] font-mono bg-amber-100 text-amber-800 font-bold px-1.5 py-0.5 rounded">STALLED</span>
                  </div>
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-4 rounded-none shadow-sm">
          ...
          <p className="text-[10px] text-[#1A1A1A]/60 mb-3 font-serif italic leading-relaxed">
            Scans individual mechanical set structures (AMRAP, Failure, Drop Sets) to prescribe exact adjustments.
          </p>
          ...
          <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
            {recentExerciseWithAnomaly.slice(0, 5).map((log, idx) => {
              ...
              return (
                <div key={idx} className="bg-[#F9F8F6] border border-[#1A1A1A]/5 p-3 rounded-none text-xs">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-[#1A1A1A] uppercase tracking-tight text-[10.5px]">{log.exerciseName}</h4>
                      <span className="text-[8.5px] font-mono text-[#1A1A1A]/40 block">{log.date}</span>
                    </div>
                    <span className="text-[8px] bg-indigo-900 text-white font-mono font-bold px-1.5 py-0.5 rounded">PATTERN DETECTED</span>
                  </div>
                  ...
                  <div className="mt-2.5 space-y-1.5">
                    {anomalySets.map((set, sIdx) => {
                      ...
                      return (
                        <div key={sIdx} className="border-l-2 border-[#E63946] pl-2.5 py-1 text-[10.5px] text-[#1A1A1A]/75 leading-relaxed font-serif italic">
                          <strong className="font-sans not-italic font-bold text-[8.5px] text-[#1A1A1A]/50 block uppercase mb-0.5">SET {sIdx + 3} Focus ({set.type})</strong>
                          {tip}
                        </div>
                      );
                    })}
  ...
  const renderVisualsTab = () => {
    // 1. GitHub contribution heatmap logic
    // Generate dates list for past 365 days grouped into 53 columns (weeks)
    ...
    const uniqueDatesWithWorkouts = Array.from(new Set(exerciseLogs.map(l => l.date))).sort();
    ...
    for (let i = 0; i < 365; i++) {
      ...
      if (workoutCountsByDate[formatted]) {
        ...
        if (!workoutCountsByDate[formattedBuffer1.toISOString().split("T")[0]] && 
    ...
    // Render cells of GitHub contribution style chart
    // To fit beautifully, we will group into 35 horizontal segments
    const renderHeatmap = () => {
      return (
        <div className="border border-[#1A1A1A]/10 p-3 bg-white rounded-none">
          ...
          <div className="flex justify-between items-center text-[9px] font-mono mt-3 text-[#1A1A1A]/50 border-t border-[#1A1A1A]/5 pt-2">
            <div className="flex gap-3">
              <span>Active Streak: <strong>{currentStreak} Days</strong></span>
              <span>Max Consistency: <strong>{maxStreak} Days</strong></span>
            </div>
    ...
    // Pre-calculate highlights data
    const maxDayVolume = Math.max(...exerciseLogs.map(l => l.sets.reduce((sum, s) => sum + (s.isWarmUp ? 0 : s.weight * s.reps), 0)));
    const highestCalculated1RM = Math.max(...exerciseLogs.map(l => Math.max(...l.sets.map(s => calculateEpley1RM(s.weight, s.reps)))));
    ...
    const flexCardsData = [
      {
        ...
        description: "You accumulated a staggering structural load in a single session, overloading muscle fibers with high mechanical stimulus.",
      ...
      },
      {
        ...
        description: "Consistency creates absolute muscle adaptation. Your physical discipline outpaced the average practitioner.",
      ...
      },
      {
        ...
        description: "Your calculated absolute failure limits highlight an elite central nervous system synchronization rate.",
      ...
      },
      {
        ...
        description: "This muscle group maintained the optimal set-rate ratios for systematic hypertrophy adaptation.",
      ...
      },
      {
        ...
        description: "Maintaining an uninterrupted physical load momentum prevents neural decline and ensures steady adaptations.",
      ...
      },
      {
        ...
        description: "You maximized your work capacity, minimizing empty rest gaps to sustain continuous muscle tension.",
      ...
      },
      {
        ...
        description: "Optimal cellular hydration ensures maximum muscle pump, fluid volume recovery, and joints cushioning.",
      ...
      },
      {
        ...
        description: "Body weight shifts demonstrate clean metabolic partitions or mass additions consistent with your program.",
      ...
      <div className="space-y-5 animate-fadeIn">
        ...
        <div className="border-b border-[#1A1A1A]/10 pb-2 mt-2">
          ...
          <p className="text-[10px] text-[#1A1A1A]/60 font-serif italic mt-0.5 leading-relaxed">
            Expand or share your physical highlights across social platforms. Crafted with athletic display layouts.
          </p>
        </div>
          ...
          <div className="fixed inset-0 bg-[#1A1A1A]/55 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#1A1A1A] text-white border border-white/10 w-full max-w-sm rounded-none overflow-hidden shadow-2xl flex flex-col">
              ...
              <div className="p-4 bg-black/20 border-t border-white/5 flex gap-2">
                ...
                >
                  {copiedShareText ? <Check className="w-3.5 h-3.5" /> : <Share2 className="w-3.5 h-3.5" />}
    ...
    <div className="flex flex-col h-full bg-[#F9F8F6] text-[#1A1A1A] overflow-y-auto p-4 md:p-6 pb-24 select-text">
      ...
      <div className="grid grid-cols-3 gap-3.5 mb-5 text-center">
        <div className="bg-white border border-[#1A1A1A]/10 p-3 shadow-sm">
          ...
          <span className="block text-[8px] uppercase font-bold text-[#1A1A1A]/40 tracking-wider">Loaded volume</span>
          <span className="text-xs font-mono font-black text-[#1A1A1A]">{coreMetrics.totalVolume.toLocaleString()}kg</span>
        </div>
        <div className="bg-white border border-[#1A1A1A]/10 p-3 shadow-sm">
          <Scale className="w-3.5 h-3.5 text-[#1A1A1A] mx-auto mb-1" />
          <span className="block text-[8px] uppercase font-bold text-[#1A1A1A]/40 tracking-wider">Current Weight</span>
        ...
        <div className="bg-white border border-[#1A1A1A]/10 p-3 shadow-sm">
          ...
          <span className="block text-[8px] uppercase font-bold text-[#1A1A1A]/40 tracking-wider">Hydration Today</span>
          <span className="text-xs font-mono font-black text-[#1A1A1A]">{(todayWaterTotal/1000).toFixed(1)}L</span>
        </div>
      </div>
      ...
      <div className="mt-8 border-t border-[#1A1A1A]/10 pt-6 space-y-6">
        <h3 className="text-[10px] uppercase font-bold tracking-[0.2em] text-[#1A1A1A]/40 mb-3">Standard Health Logger</h3>
        ...
        <div className="bg-white border border-[#1A1A1A]/10 rounded-none p-4 shadow-sm">
          ...
          <div className="flex items-center gap-4 py-1">
            <div className="relative w-12 h-18 border border-[#1A1A1A]/20 bg-[#F9F8F6] overflow-hidden flex flex-col justify-end">
              <div style={{ height: `${waterPercent}%` }} className="w-full bg-[#E63946] transition-all" />
        ...
        <div className="bg-white border border-[#1A1A1A]/10 rounded-none p-4 shadow-sm">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-[9px] font-mono text-[#E63946] font-bold">
              {weightDiff >= 0 ? "+" : ""}{weightDiff.toFixed(1)}kg Over Time
            </span>
          </div>
          ...
          <div className="bg-[#F9F8F6] border border-[#1A1A1A]/20 w-full max-w-sm rounded-none overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="bg-[#1A1A1A] text-white p-4 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-serif italic font-bold text-base uppercase tracking-wider">Log Manual Training Set</h3>
              </div>
              <button 
            ...
            <form onSubmit={handleLogCustomSetSubmit} className="p-4 space-y-3.5">
              ...
              {/* Category selector */}
              <div>
                <label className="block text-[8px] font-bold uppercase tracking-wider text-[#1A1A1A]/50 mb-1">Muscle Category</label>
                <select
              ...
              <div>
                <label className="block text-[8px] font-bold uppercase tracking-wider text-[#1A1A1A]/50 mb-1">Exercise Name</label>
                <select
                  ...
                  className="w-full bg-white border border-[#1A1A1A]/15 px-2.5 py-1.5 text-xs focus:outline-none"
                >
                  {EXERCISE_DATABASE.filter(e => e.targetMuscle.toLowerCase() === logExMuscle.toLowerCase() || 
                    (logExMuscle === "Core" && ["Core", "Lower Abs", "Obliques"].includes(e.targetMuscle)) ||
                    (logExMuscle === "Lats" && ["Lats", "Mid Back", "Upper Back"].includes(e.targetMuscle)) ||
                    (logExMuscle === "Mid Back" && ["Lats", "Mid Back", "Upper Back"].includes(e.targetMuscle)) ||
                    (logExMuscle === "Upper Back" && ["Lats", "Mid Back", "Upper Back"].includes(e.targetMuscle)) ||
                    (logExMuscle === "Quads" && ["Quads", "Hamstrings", "Glutes"].includes(e.targetMuscle)) ||
                    (logExMuscle === "Hamstrings" && ["Quads", "Hamstrings", "Glutes"].includes(e.targetMuscle)) ||
                    (logExMuscle === "Glutes" && ["Quads", "Hamstrings", "Glutes"].includes(e.targetMuscle))
                  ).map((e) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[8px] font-bold uppercase tracking-wider text-[#1A1A1A]/50 mb-1">Weight Load (kg)</label>
                  <input 
                ...
                <div>
                  <label className="block text-[8px] font-bold uppercase tracking-wider text-[#1A1A1A]/50 mb-1">Repetitions</label>
                  <input 
              ...
              <div className="grid grid-cols-2 gap-3 items-center pt-1">
                <div>
                  <label className="block text-[8px] font-bold uppercase tracking-wider text-[#1A1A1A]/50 mb-1">Set Intensity Type</label>
                  <select
                ...
                <div className="flex items-center gap-2 pt-4">
                  ...
                  />
                  <label className="text-[10px] font-bold uppercase text-[#1A1A1A]/60">Warm-Up Set</label>
                </div>
              </div>
